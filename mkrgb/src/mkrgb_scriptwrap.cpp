...
int main(int argc, char* argv[]) {
  if (argc < 3) {
    cout << "Magic Knight Rayearth (GB) script wrapper" << endl;
  ...
  string tableName = "table/mkrgb_en.tbl";
...
//  table.readUtf8(tableName.c_str());
  ...
  {
    // read size table
    MkrGbLineWrapper::CharSizeTable sizeTable;
    {
      TBufStream ifs;
...
/*    MkrGbLineWrapper::CharSizeTable sizeTableEmph;
    {
      TBufStream ifs;
    ...
    
    {
      ...
      MkrGbLineWrapper(ifs, results, table, sizeTable, sizeTable,
                       isNonstd)();
